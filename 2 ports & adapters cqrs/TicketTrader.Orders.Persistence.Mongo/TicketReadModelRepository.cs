...
{
    class TicketReadModelRepository: MongoReadSideRepository<TicketOrderReadModel>, ITicketFinder, IOrderTicketsDenormalizer
    {
        public TicketReadModelRepository(MongoReadSideRepositoryContext context) : base(context)
        {
        }

        public async Task<IEnumerable<TicketOrderReadModel>> GetClientTicketForEventAsync(string clientId, string orderId, string eventId)
        {
            var result = await Collection.FindAsync(x => x.OrderId == orderId && x.EventId == eventId);
            return await result.ToListAsync();
        }

        public async Task<IEnumerable<TicketOrderReadModel>> GetClientTicketsAsync(string clientId, string orderId)
        {
        ...
        {
            var readModel = new TicketOrderReadModel
            {
