...
export class PlanVisualizationComponent implements OnInit, OnDestroy {
  ...
  userReservations: SeatReservationDto[];
  ...
  ngOnInit() {
    ...
    this.reservedSeatsSubscription = this.eventIdSubject
      .asObservable()
      .switchMap(eventId => this.reservationService.getAllReservedSeats(eventId.toString()))
      .subscribe(seats => {
    ...
    this.userReservationsSubscription = this.withOrderContext()
      .switchMap(context => {
        return this.clientReservationsService.getClientReservations(context.eventId.toString(), context.client.clientId, context.order.id)
      })
    ...
    this.removeReservationRequestSubscription = this.messageBus
      ...
      .subscribe(message => {
        message.sceneSeatIds.forEach(sceneSeatId => {
          const seatNode: SeatNode = this.map.getSeatNodeById(Number(sceneSeatId));
    ...
    this.canceledTicketReservations = this.messageBus
      ...
      .subscribe(message => {
        this.discardSeat(message.eventId, Number(message.sceneSeatId));
      });
  }
  ...
  reserveSeat(seat: SeatNode) {
    this.withOrderContext()
      .switchMap(context => this.clientReservationsService.createClientReservation(
        ...
        context.order.id,
        seat.getSceneSeatId().toString()))
      .subscribe(response => {
        seat.markAsSelected();
        this.userSelectedSeats.push(seat.getSceneSeatId().toString());
  ...
  discardSeatReservation(seat: SeatNode, emit: boolean = true) {
    this.withOrderContext()
      .subscribe(response => {
        seat.markAsFree();
        const index = this.userSelectedSeats.indexOf(seat.getSceneSeatId().toString(), 0);
        if (index > -1) {
        ...
        this.withOrderContext()
          .subscribe(context => {
            if (emit) {
              this.messageBus.publish(new SeatDiscardRequest(context.eventId, seat.getSceneSeatId().toString()));
            }
          });
      });
  }
  ...
  discardSeat(eventId: number, sceneSeatId: number): void {
    let reservationToDiscard = this.userReservations.filter(s => s.sceneSeatId == sceneSeatId.toString())
debugger;
    if(reservationToDiscard.length == 0){
      return;
    }

    let reservationId = reservationToDiscard[0].reservationId
