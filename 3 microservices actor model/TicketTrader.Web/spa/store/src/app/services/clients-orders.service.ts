...
export class ClientsOrdersService {
  ...
  getOrCreateActiveOrder(clientId: string): Observable<ClientOrderDto> {
  ...
  createOrder(clientId: string): Observable<ClientOrderDto> {
    const service = this.clientsFactory.createClientsOrdersService();
    return service
      .post(clientId)
      .map(response => {
  ...
  commitOrder(clientId: string, orderId: string): Observable<boolean> {
    const service = this.clientsFactory.createClientsOrdersService();
    return service
