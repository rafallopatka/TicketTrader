...
export class ReservationsPadComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.discardeddSeatsSubscription = this.messageBus
      ...
      .subscribe(message => {
        const ticket = this.tickets.find(t => t.sceneSeatIds.findIndex(s => s === message.sceneSeatId) !== -1);
        if (ticket != null) {
    ...
    this.reserveTicketRequestsSubscription = this.messageBus
      ...
      .switchMap(message => {
        return this.orderTicketsService.orderTicket(message.eventId,
          message.clientId,
          message.orderId,
          message.sceneSeatId,
    ...
    this.eventOrdersSubscription = this.withOrderContext()
      .switchMap(context => this.orderTicketsService.getClientTicketsForEvent(context.eventId, context.client.clientId, context.order.id))
      .subscribe(tickets => {
