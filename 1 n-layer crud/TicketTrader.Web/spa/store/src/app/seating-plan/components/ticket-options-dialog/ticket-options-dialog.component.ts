...
export class TicketOptionsDialogComponent extends BaseComponent implements OnInit, OnDestroy {
  context: { eventId: number; clientId: number; orderId: number; sceneSeatId: number; scenePriceZoneId: number; selectedOption: number; };
  ...
  discard() {
    this.messageBus
      .publish(new TicketReservationCancelRequest(this.context.eventId, this.context.sceneSeatId));
  }
